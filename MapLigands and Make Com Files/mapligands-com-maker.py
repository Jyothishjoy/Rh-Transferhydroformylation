...
def process_xyz_files(directory, output_file, header_template_path, footer_template_path):
    with open(header_template_path, 'r') as header_template:
        header_content = header_template.read()
        ...
        for filename in files:
            if filename == output_file:
                ...
                with open(in_file, 'w') as in_template:
                    in_template.write(header_content)
...
template_paths = {
    'OxiAdnTS': (os.path.join(main_dir, 'OxiAdnTS_header.txt'), os.path.join(main_dir, 'OxiAdnTS_footer.txt')),
    'AcylTS': (os.path.join(main_dir, 'AcylTS_header.txt'), os.path.join(main_dir, 'AcylTS_footer.txt')),
    'betaHTS': (os.path.join(main_dir, 'betaHTS_header.txt'), os.path.join(main_dir, 'betaHTS_footer.txt')),
    'COextTS': (os.path.join(main_dir, 'COextTS_header.txt'), os.path.join(main_dir, 'COextTS_footer.txt')),
}
...
for xyz_file in xyz_files:
    ...
    if 'OxiAddn-TS' in xyz_file:
        header_path, footer_path = template_paths['OxiAdnTS']
    elif 'Acyl-TS' in xyz_file:
        header_path, footer_path = template_paths['AcylTS']
    elif 'b-hydride-Elim-TS' in xyz_file:
        header_path, footer_path = template_paths['betaHTS']
    elif 'CO-extrusion-TS' in xyz_file:
        header_path, footer_path = template_paths['COextTS']
    else:
        # Fallback or default case
        header_path, footer_path = os.path.join(main_dir, 'inp_template_header.txt'), os.path.join(main_dir, 'inp_template_footer.txt')

    # Process the .xyz files
    process_xyz_files(main_dir, output_file, header_path, footer_path)
